...
def frus_editorial_cleanup(raw_body: str) -> tuple[str, dict[str, Any]]:
    ...
    for line_no, raw_line in enumerate((raw_body or "").splitlines(), start=1):
        ...
        line = re.sub(r"\((?:PX|Pf|P|J|§|f|Z|£|9|\$)\)", "(S)", line)
        line = re.sub(r"\bnucleararmed\b", "nuclear-armed", line, flags=re.I)
        line = re.sub(r"^dh\s+(?=ALCMs and SLCMs\b)", "1. ", line)
        line = re.sub(r"^CG\.\s+(?=A statement\b)", "c. ", line)
        line = re.sub(r"^di\.\s+(?=Information\b)", "d. ", line)
        line = re.sub(r"^Bs\s+(?=Verifying\b)", "2. ", line)
        line = re.sub(r"^lor\s+(?=Up to\b)", "a. ", line)
        line = re.sub(r"^om\s+(?=The Soviet facilities\b)", "b. ", line)
        line = re.sub(r"^3s\s+(?=Limits on Heavy ICBMs\b)", "3. ", line)
        line = re.sub(r"^ole\s+(?=Such limitations\b)", "d. ", line)
        line = re.sub(r"\(da\)", "(d)", line)
