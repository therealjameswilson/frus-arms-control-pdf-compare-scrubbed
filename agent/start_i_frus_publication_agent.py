#!/usr/bin/env python3
"""Build a reviewable FRUS START I publication packet from one Bush PDF.

The runner is deliberately conservative. It can reconstruct a known
PDF/FRUS pair from this repository or process a new PDF with editor-supplied
metadata. It emits evidence and review warnings rather than pretending OCR is
...
AGENT_NAME = "START_I_FRUS_PUBLICATION_AGENT"
REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_DATA = REPO_ROOT / "assets" / "data" / "frus-pdf-compare.json"
DEFAULT_CACHE = Path.home() / ".cache" / "frus-start-publication-agent"
DEFAULT_OUTPUT = Path(tempfile.gettempdir()) / "frus-start-publication-agent-output"
...
def select_known_row(payload: dict[str, Any], args: argparse.Namespace) -> dict[str, Any] | None:
...
def resolve_pdf(pdf_arg: str, cache_dir: Path) -> tuple[Path, str]:
    if re.match(r"^https?://", pdf_arg):
        ...
        if not dest.exists():
            req = urllib.request.Request(pdf_arg, headers={"User-Agent": "FRUS START publication agent"})
...
PAGE_CLASS_PATTERNS: list[tuple[str, list[str]]] = [
    ("administrative_marker", ["administrative marker", "record group/collection", "folder title", "oa/id number"]),
    ("withdrawal_sheet", ["withdrawal/redaction sheet", "restriction", "foia/sys case"]),
    ("access_control", ["attached document contains classified", "access list"]),
    ("routing_profile", ["nsc profile", "source data page", "action officer", "status"]),
    ("distribution_record", ["distribution record", "directorate distribution", "external distribution"]),
    ("source_document", ["the white house", "memorandum for", "subject:", "national security review", "national security directive", "summary of conclusions"]),
]
...
def build_packet(args: argparse.Namespace) -> dict[str, Any]:
    ...
    known_row = select_known_row(payload, args)
    ...
    if known_row:
        ...
        source_note = args.source_note or doc.get("source_note") or known_row.get("source_note") or ""
        model_text = strip_html(doc.get("html", ""))

    if not pdf_source:
        raise SystemExit("Supply --pdf or select a known row with --doc-no, --doc-key, or --row-id")
    ...
    if not known_row and not args.page_range:
        warnings.append("New-source run without explicit page range; page span requires human confirmation.")
    ...
    return {
        "agent": AGENT_NAME,
        "run_mode": "known_pair_reconstruction" if known_row else "new_source_draft",
        "source_pdf": {
            "source": source_label,
        ...
        "reverse_engineered_process": [
            "inventory source PDF and source-register evidence",
            "OCR image-only file-unit pages with page provenance",
            "classify administrative, withdrawal, access, routing, distribution, and source-document pages",
            "select one document span from the file-unit PDF",
            "omit archival scaffolding from body text while preserving provenance evidence",
            "normalize OCR into FRUS-style body paragraphs",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create a FRUS START I publication packet from one Bush PDF.")
    ...
    parser.add_argument("--doc-no", help="Known FRUS document number, e.g. 10.")
    parser.add_argument("--pdf", help="PDF path or URL for new-source runs.")
    parser.add_argument("--volume-id", default="frus1989-92v31", help="Target volume id for new-source runs.")
    parser.add_argument("--title", help="Provisional title for new-source runs.")
    parser.add_argument("--source-note", help="Source note or source-register citation for new-source runs.")
